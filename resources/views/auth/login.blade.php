...
<head>
    ...
    <title>{{ __('messages.login_page_title') }} - {{ config('app.name') }}</title>
            ...
            <form action="{{ route('login') }}" method="POST">
                ...
                <div class="mb-3 form-check d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <label for="remember" class="form-check-label">{{ __('messages.login_remember') }}</label>
                    </div>
                    <a href="{{ route('password.request') }}" class="small">{{ __('messages.login_forgot') }}</a>
                </div>
                <button type="submit" class="btn btn-primary w-100">{{ __('messages.login_submit') }}</button>
            </form>
        </div>
    </div>

    <div class="text-center mt-3">
        <p class="mb-1">{{ __('messages.login_with') }}</p>
